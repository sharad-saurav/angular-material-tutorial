...
export class EditConfigComponent implements OnInit {
  ...
  onItemSelect(item){
    this.configArray.forEach(element => {
      if(element.rule == item){
        this.selectedItems = [];
        this._snackBar.open("This Rule is Already Added Please delete first to make changes on it", '', {
          duration: 5000,
        });
        return;
      }
  ...
  addRule(){
    if(this.selectedColumns.length == 0 || this.selectedFiles.length == 0){
      this._snackBar.open("please first select columns and files for the rule", '', {
        duration: 5000,
      });
      return;
    }
    ...
    if(this.selectedFiles.length == 6){
      this.configArray.push({"rule": this.selectedItems[0].text, "columnsToApply":selectColumn, "filesToApply": "ALL"})
    }else{
      this.configArray.push({"rule": this.selectedItems[0].text, "columnsToApply":selectColumn, "filesToApply":selectFile})
    }
    ...
    if(this.configArray.length != 0){
      this.imageKey = true;
        ...
        this.httpClient.post('api/changeConfig', data).subscribe(data =>{
          ...
          this._snackBar.open("config file changed please download and check the changes", '', {
            duration: 5000,
          });
        },
        error =>{ console.log('oops', error)
        ...
        this._snackBar.open("please add changes first", '', {
          duration: 5000,
        });
    }
  } 
